...
export const BurgerStructureItem = ({
  ingredient,
  isTopBun = false,
  isBottomBun = false,
}: TBurgerStructureItemProps): React.JSX.Element => {
  const { name, price, image_mobile } = ingredient;
  const isBun = isTopBun || isBottomBun;
  const ingredientClasses = [styles.item_inner, 'pt-4 pb-4 pr-8 pl-6'];
  if (isTopBun) {
  ...
  return (
    <article className={styles.item}>
      <span className={isBun ? 'pr-8' : 'pr-2'}>
